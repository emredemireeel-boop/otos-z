...
const signFileMap = {
    ...
    // TRAFIK TANZIM
    'TT-1': 'Turkey road sign B-2a.svg',
    'TT-2': 'Turkey road sign B-3.svg',
...
function getImageUrl(filename) {
    return new Promise((resolve, reject) => {
        ...
        https.get(apiUrl, { headers: { 'User-Agent': 'OtoAsfaltBot/1.0' } }, (res) => {
            let data = '';
...
async function main() {
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        // Let's check if the file size is very small (my fake SVGs were tiny, real ones are larger, usually > 1KB)
        // Or actually, let's just re-download them all to be absolutely sure we have the real ones, 
        // since my previous script overwrote the first 41 with real ones, but the last ones are still fake.
        
        console.log(`[${i+1}/${entries.length}] ${signId}`);
