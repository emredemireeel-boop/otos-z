...
import { createSeoSlug as createVehicleSlug } from "@/lib/slug";
...
const MarkdownEditor = dynamic(() => import("@/components/MarkdownEditor"), {
    ssr: false,
    loading: () => <div className="new-topic-editor-loading">Editör yükleniyor…</div>,
});
...
type HomeMyth = { id: number; slug: string; myth: string };

const HOME_MYTHS: HomeMyth[] = [
    { id: 1, slug: "yokus-asagi-bosa-atmak", myth: "Yokuş aşağı inerken vitesi boşa almak yakıt tasarrufu sağlar." },
    { id: 2, slug: "klima-yerine-cam-acmak", myth: "Klimayı kapatıp camları açmak her zaman daha ekonomiktir." },
    { id: 3, slug: "benzin-deposunu-fullemek", myth: "Benzin deposunu tamamen doldurmak aracı çok daha fazla yaktırır." },
    { id: 4, slug: "sabah-motoru-isitmak", myth: "Sabah motoru ısıtmak için 5-10 dakika rölantide beklemek gerekir." },
    { id: 5, slug: "yuksek-oktanli-benzin-performansi", myth: "Yüksek oktanlı benzin her otomobilin performansını artırır." },
    { id: 6, slug: "kalin-yag-motoru-korur", myth: "Kalın motor yağı her motoru daha iyi korur." },
    { id: 7, slug: "motor-yikanmaz", myth: "Modern araç motorları tazyikli suyla güvenle yıkanabilir." },
    { id: 8, slug: "farkli-antifriz-karisimi", myth: "Farklı renkteki antifrizler sorunsuzca karıştırılabilir." },
];

const FEATURED_DNA_MODELS = [
    { id: 1, brand: "Renault", model: "Clio 5. Nesil (2020-2025)", dnaScore: 78, totalReports: 7, chronicIssueCount: 2 },
    { id: 2, brand: "Fiat", model: "Egea 1. Nesil (2015-2025)", dnaScore: 75, totalReports: 6, chronicIssueCount: 2 },
    { id: 3, brand: "Toyota", model: "Corolla 12. Nesil (2019-2025)", dnaScore: 85, totalReports: 9, chronicIssueCount: 4 },
    { id: 4, brand: "Renault", model: "Megane 4. Nesil (2016-2025)", dnaScore: 76, totalReports: 9, chronicIssueCount: 2 },
    { id: 5, brand: "Honda", model: "Civic 11. Nesil FE1 (2021-2025)", dnaScore: 82, totalReports: 6, chronicIssueCount: 3 },
    { id: 6, brand: "Volkswagen", model: "Passat B8 (2015-2023)", dnaScore: 84, totalReports: 8, chronicIssueCount: 2 },
    { id: 7, brand: "Dacia", model: "Duster 2. Nesil (2018-2024)", dnaScore: 73, totalReports: 6, chronicIssueCount: 3 },
    { id: 8, brand: "Hyundai", model: "i20 3. Nesil BC3 (2020-2025)", dnaScore: 77, totalReports: 8, chronicIssueCount: 5 },
    { id: 13, brand: "Volkswagen", model: "Golf 8. Nesil (2020-2025)", dnaScore: 80, totalReports: 6, chronicIssueCount: 2 },
];

const HOME_FEATURED_EVENT = {
...
export default function Home({ decisionHub }: { decisionHub: ReactNode }) {
    ...
    const [carModelsData, setCarModelsData] = useState<Record<string, string[]>>({});
    ...
    const [randomMyth, setRandomMyth] = useState<HomeMyth | null>(HOME_MYTHS[0]);
    ...
    useEffect(() => {
        ...
        // Hafif, öne çıkarılmış efsane listesi istemci paketini büyütmez.
        const randomMythIndex = Math.floor(Math.random() * HOME_MYTHS.length);
        setRandomMyth(HOME_MYTHS[randomMythIndex]);
    }, []);

    useEffect(() => {
        if (!showNewTopicModal || Object.keys(carModelsData).length > 0) return;
        let active = true;
        import("@/data/carmodels.json")
            .then(module => {
                if (active) setCarModelsData(module.default as Record<string, string[]>);
            })
            .catch(() => {
                if (active) setCarModelsData({});
            });
        return () => { active = false; };
    }, [showNewTopicModal, carModelsData]);
    ...
    const featuredEvent = HOME_FEATURED_EVENT;
    return (
        <div>
            <script
            ...
            <main className="forum-home-main">
                {/* Sub Header */}
                <div className="subheader-container">
                    <div className="forum-subheader-inner">
                        <div className="subheader-row forum-subheader-row">
                            ...
                            {/* Fuel Widget */}
                            <div className="fuel-ticker-wrapper glass">
                                <div className="ticker-track">
                                    {/* Şehir etiketi */}
                                    <div className="ticker-city-wrap">
                                    ...
                                    {[...fuelPrices, ...fuelPrices].map((fuel, i) => (
                                        <div key={i} className="ticker-price-item">
                                            <span className="ticker-price-name">{fuel.name}</span>
                                            <span className="ticker-price-value">{fuel.price}</span>
                                            <span className={fuel.trend === 'up' ? 'trend-up' : fuel.trend === 'down' ? 'trend-down' : 'trend-stable'}>
                            ...
                            <div className="forum-sidebar-stack">
                                        ...
                                        {FEATURED_DNA_MODELS.map((vehicle) => (
                                            ...
                                            >
                                                <span className="forum-dna-copy">
                                                    ...
                                                    <small>{vehicle.chronicIssueCount} kronik konu · {vehicle.totalReports} kullanıcı raporu</small>
                                                </span>
                                ...
                                {featuredEvent && (
                                    <section className="forum-sidebar-card forum-event-card forum-sidebar-bottom">
                                        ...
                                        <Link href={featuredEvent.href} className="forum-card-link">
                ...
                <section className="home-seo-guide" aria-labelledby="home-seo-guide-title">
                    <div className="home-seo-guide-heading">
                        <span>OtoSöz karar rehberi</span>
                        <h2 id="home-seo-guide-title">Araç alımı, arıza çözümü ve otomobil karşılaştırma rehberi</h2>
                        <p>Karar vermeden önce gerçek sürücü deneyimini, uzman bilgisini ve düzenli araç verisini aynı yerde inceleyin.</p>
                    </div>
                    <div className="home-seo-guide-grid">
                        <article>
                            <h3>Araba alırken doğru karar</h3>
                            <p>Araç DNA dosyalarında kronik sorunları, kullanıcı raporlarını ve model bazlı güçlü-zayıf yönleri karşılaştırın.</p>
                            <Link href="/arac-dna">Araç DNA analizlerini incele <ArrowRight size={14} /></Link>
                        </article>
                        <article>
                            <h3>Otomobil arızasını doğru tanı</h3>
                            <p>Belirtileri uzmanlara sorun; OBD arıza kodları ve bakım rehberleriyle olası nedeni araştırın.</p>
                            <Link href="/obd">OBD arıza kodlarını bul <ArrowRight size={14} /></Link>
                        </article>
                        <article>
                            <h3>İki aracı verilerle karşılaştır</h3>
                            <p>Yakıt, kullanım maliyeti, sürücü görüşü ve topluluk oyuyla seçeneklerinizi yan yana değerlendirin.</p>
                            <Link href="/karsilastirma">Araç karşılaştırmaya başla <ArrowRight size={14} /></Link>
                        </article>
                    </div>
                </section>
            </main>
